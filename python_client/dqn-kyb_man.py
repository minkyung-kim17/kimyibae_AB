...
# actions_idx[4] = [23]
# actions_idx[5] = [22, 30]
# actions_idx[6] = [21, 26] # 2shots
# actions_idx[7] = [23]
# actions_idx[8] = [31] # 2shots ? 
...
EXP_PATH=os.path.join(current_dir,"experiences-manData") 
...
while True:
	...
	# break
		...
		for t in itertools.count(): # 이 에피소드가 끝날때까지
			# continue_flag = int(input("continue_flag(0)(1:go to other level):"))
			# if continue_flag == 1:
			...
			# print('angle:')
			# angle_action_idx = int(input("angle_idx:"))
			# taptime_action_idx = int(input("taptime_idx:"))

			# angle_action_idx = valid_action.actions[current_level][0]
			# taptime_action_idx = 
			...
			angle_action = int(input("angle:"))
			taptime_action = int(input("taptime:"))
			...
			print("ref_point[0]: {}, ref_point[1]: {}, dx: {}, dy: {}".format(ref_point[0], ref_point[1], dx, dy), end="\n")
