...
checkpoint_dir = os.path.join(current_dir, "checkpoints_parNN_feed_thread")
...
run_start_dir = os.path.join(EXP_PATH, "startAt_%s"%start_time)
...
angle_estimator = q_network.DQN_Estimator(scope="angle_estimator", output_size=len(valid_angles), summaries_dir=SUMM_PATH, level_as_state=21)
angle_target_estimator = q_network.DQN_Estimator(scope="angle_target_estimator", output_size=len(valid_angles),level_as_state=21)
taptime_estimator = q_network.DQN_Estimator(scope="taptime_estimator", output_size=len(valid_taptimes), summaries_dir=SUMM_PATH, angle_as_state = len(valid_angles),level_as_state=21)
taptime_target_estimator = q_network.DQN_Estimator(scope="taptime_target_estimator", output_size=len(valid_taptimes), angle_as_state = len(valid_angles),level_as_state=21)
...
with tf.Session() as sess:
	...
	try:
		...
		memory_list = glob.glob('replay_memory*.npy')
	...
	while True:
		...
		if game_state=='UNKNOWN':
			...
			level_onehot[current_level-1] = 1
			...
			for t in itertools.count(): # 이 에피소드가 끝날때까지
				...
				state = np.concatenate((level_onehot,dqn_utils.get_feature_4096(model=vgg16, img_path=save_path)))
				# pdb.set_trace()
				...
				next_state = np.concatenate((level_onehot,dqn_utils.get_feature_4096(model=vgg16, img_path=save_path)))
				...
				new_memory.append(Transition(state, action, reward, next_state, game_state))


				# total_t = sess.run(tf.train.get_global_step())
				if game_state=='WON' or game_state=='LOST':
					print("===numpy save===please wait===")
					np.save('replay_memory_%s.npy'%(start_time), new_memory) # 저장을 쓰레드 안으로 옮김!
					print('===numpy save===')
					if len(new_memory) > 100:
						print("new_memory length > 100 : make new npy file for the next time.")
						sys.stdout.flush()
						new_memory = []
						start_time = time.strftime("%Y%m%d_%H%M")
					sys.stdout.flush()
				...
				if len(new_memory) > batch_size_playing:
					angle_loss, taptime_loss = dqn_utils.pretrain_parNN(new_memory, valid_angles, valid_taptimes, angle_estimator, taptime_estimator, angle_target_estimator, taptime_target_estimator, sess, batch_size_playing, discount_factor, angle_feed = True)
