...
def get_state(screenshots_list, state_name, playing_state):
	...
	if (length == 1):
		if playing_state == 'FIRST_SHOT':
			gathering_logger.debug("First shot... Stack images.")
			try:
				cropped.append(Image.open(current_dir+"/all_black.png"))
				cropped.append(Image.open(screenshots_list[0]).crop((70,45,760,390)))
			except Exception as e:
				gathering_logger.error("Error occurs at stacking two images.")
				gathering_logger.error(str(e))
		elif playing_state == 'FINAL_SHOT':
			gathering_logger.debug("Final shot... Stack images.")
			try:
				cropped.append(Image.open(screenshots_list[0]).crop((70,45,760,390)))
				cropped.append(Image.open(current_dir+"/all_black.png"))
			except Exception as e:
		...
		if playing_state == 'AMONG_SHOTS':
			gathering_logger.debug("Keep shooting... Stack images.")
			try:
			...
			gathering_logger.error("playing_state error, current state is ", playing_state)
	else:
		gathering_logger.error("More than two images in the list.")
	...
	try:
		...
		gathering_logger.error("Error occurs at saving stacked image.")
		gathering_logger.error(str(e))
